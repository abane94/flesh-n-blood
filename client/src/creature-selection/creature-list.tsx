...
import { setCreatureCard } from "../global-state";
...
const CreatureList = (props: Props) => {
    console.log(props.cards);

    return (
        <div class="tab-content" >
            <div class="flex 2">
                <For each={props.cards}>{(card, i) =>
                    <div class="card" onClick={() => setCreatureCard(card)}>
                        {card.name}
                        <img src={card.cardImage} class="" />
                    </div>
                }</For>
            </div>
        </div>
